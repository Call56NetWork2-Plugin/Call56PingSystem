package com.github.kinoko2k.ping;
        ...
        if (cmd.getName().equalsIgnoreCase("ping"))
        {
            ...
            if(args.length == 0) {
                ...
                if (!(sender instanceof Player)){
                    sender.sendMessage(prefix + " " +  "あなたのping: " + ChatColor.GRAY + "0 ms" + ChatColor.RESET + " ...あなたコンソール見てみたら？");
                    return true;
                } else {
                    if(sender.hasPermission("ping.self")){
                        // Define player object
                        final Player p = (Player) sender;
                        sender.sendMessage(prefix + " " + "あなたのping: " + ChatColor.GRAY + getPing(p) + " ms" + ChatColor.RESET);
                        return true;
                    } else {
                        sender.sendMessage(prefix + " " +  "あなたに権限はありません！！: " + ChatColor.GRAY + "- ping.self");
                        return true;
                    }
                }
                ...
                if(sender.hasPermission("ping.others")){
                    ...
                    sender.sendMessage(prefix + " " +  "あなたに権限はありません！！: " + ChatColor.GRAY + "- ping.others");
                    return true;
                }
            } else {
                // Send command overview
